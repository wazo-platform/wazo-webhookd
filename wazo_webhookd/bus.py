...
class CoreBusConsumer(kombu.mixins.ConsumerMixin):
    ...
    def get_consumers(self, Consumer, channel):
        logger.debug('Acquiring consumers lock for listing consumers...')
    ...
    def on_connection_error(self, exc, interval):
        ...
        logger.debug('Acquiring consumers lock for connection error...')
        with self._consumers_lock:
    ...
    def subscribe_to_event_names(
        ...
        if uuid is None:
            ...
            return
        logger.debug('Acquiring consumers lock for creation (uuid: %s)...', uuid)
        with self._consumers_lock:
            logger.debug(
    ...
    def change_subscription(self, uuid, event_names, user_uuid, wazo_uuid, callback):
        if uuid is None:
            ...
            logger.warning("subscription `%s` doesn't have event_names set", uuid)
            return
        logger.debug('Acquiring consumers lock for update (uuid: %s)...', uuid)
        with self._consumers_lock:
    ...
    def unsubscribe_from_event_names(self, uuid):
        if uuid is None:
            raise RuntimeError("uuid must be set")
        logger.debug('Acquiring consumers lock for deletion (uuid: %s)...', uuid)
        with self._consumers_lock:
